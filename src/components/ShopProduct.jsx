...
function ShopProduct({
  ...
  const className = productType
    ? "shopProduct shopProduct-type1"
    : "shopProduct shopProduct-type2"
        ...
        <div className="">
            ...
            <div className="">
              {Array.isArray(price) && price.length > 0 ? (
                price.length > 1 && price[0] !== price[1] ? (
                  <span className="productPrice">{`${parseFloat(
                    price[0]
                  ).toFixed(2)}som-${parseFloat(price[1]).toFixed(
                    2
                  )}som`}</span>
                ) : (
                  <span className="productPrice">{`${parseFloat(
          ...
          <div className="addToCartShop">
            {hasOption ? (
              ...
              <Link
                to={`/shop/add-to-cart/${encodedTitle}/${"size_none"}/${"addon_none"}`}
              >
